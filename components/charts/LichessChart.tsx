...
const CACHE_KEY = 'lichess_metrics_cache';
const CACHE_DURATION = 1000 * 60 * 5; // 5 minutes cache

export default function LichessChart() {
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        ...
        const response = await fetch('/api/metrics?source=lichess&range=30');

        if (!response.ok) {
          throw new Error('Failed to fetch Lichess metrics');
        }
        ...
        console.log('Lichess metrics loaded:', {
        ...
        console.error('Error fetching Lichess metrics:', err);
        setError('Failed to load Lichess data');
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="text-center py-8">Loading Lichess data...</div>;
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <LineChart data={data}>
        ...
        />
        <Legend />
        <Line type="monotone" dataKey="bullet_rating" stroke="#ff6b6b" name="Bullet" connectNulls />
        <Line type="monotone" dataKey="blitz_rating" stroke="#4ecdc4" name="Blitz" connectNulls />
        <Line type="monotone" dataKey="rapid_rating" stroke="#45b7d1" name="Rapid" connectNulls />
